import { searchOpenFoodFacts } from './clients/open-food-facts-client';
import { searchUsda, UsdaResult } from './clients/usda-client';

export interface FoodSearchResolution {
  name: string;
...
async function searchOffTagged(query: string): Promise<FoodSearchResolution[]> {
  const results = await searchOpenFoodFacts(query);
  return results.map((result) => ({ ...result, source: 'openfoodfacts' as const }));
}

async function searchUsdaTagged(query: string, apiKey: string): Promise<FoodSearchResolution[]> {
...
export async function resolveFoodSearch(
  query: string,
  usdaApiKey: string,
): Promise<FoodSearchResolution[]> {
  const brandFirst = hasBrand(query);
  const primary = brandFirst ? () => searchOffTagged(query) : () => searchUsdaTagged(query, usdaApiKey);
